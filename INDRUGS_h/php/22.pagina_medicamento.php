...
if (!isset($_SESSION['ID_USUARIOS'])) {
    ...
    m.DESCRIPCION_MEDICAMENTOS,
    i.STOCK_INVENTARIO,
    i.ESTADOMED_INVENTARIO
    FROM medicamentos m
    LEFT JOIN inventario i ON m.ID_MEDICAMENTOS = i.ID_MEDICAMENTOS;";
...
<body>
    ...
    <main class="cuerpo_indrugs">
        <div class="contenido">
            <h1 class="tabla_usuarios">Medicamentos</h1>
            <?php  if($result->num_rows > 0){
                ...
                $estadoTexto = strtoupper($row['ESTADOMED_INVENTARIO']) === 'ACTIVO' ? 'Activo' : 'Inactivo';
                    echo "
                    <tr>
                        <td>{$row['ID_MEDICAMENTOS']}</td>
                        <td>{$row['NOMBRE_MEDICAMENTOS']}</td>
                        <td>{$row['DESCRIPCION_MEDICAMENTOS']}</td>
                ...
                <a href="../23.agregar_medicamentos.html"><button class="btn btn-primary">Agregar medicamento</button></a>
        </div>    
    </main>
